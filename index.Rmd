...
```{r}
scholar_html <- read_html("https://scholar.google.com/citations?user=KmIt5HIAAAAJ&hl=en")
```
...
article_titles <- publications %>% html_nodes("td.gsc_a_t > a") %>% html_text() %>% as.character()
citations <- publications %>% html_nodes("td.gsc_a_c > a") %>% html_text() %>% as.integer()
years <- publications %>% html_nodes("td.gsc_a_y > span") %>% html_text() %>% as.integer()
